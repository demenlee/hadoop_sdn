...
net = Containernet(controller=RemoteController)   # 控制器ryu应用编写 要注意 拓扑环路的处理
...
for i in range(16):
    ...
    if i == 1:
        host_name['h' + str(i)] = net.addDocker(str_hosts[i], ip=str_ips[i], ports=['50070', '8088', '9000'] , 
                                    dimage="demenlee/hadoop:v3.2.master", mac=mac_adrs[i])
    else:
        host_name['h' + str(i)] = net.addDocker(str_hosts[i], ip=str_ips[i], dimage="demenlee/hadoop:v3.2", mac=mac_adrs[i])
